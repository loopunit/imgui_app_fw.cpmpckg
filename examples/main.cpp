...
int main(int, char**)
{
	return mu::leaf::try_handle_all(
		...
		[]() -> mu::leaf::result<int> {
			BOOST_LEAF_CHECK(imgui_app_fw()->select_platform());

			BOOST_LEAF_CHECK(imgui_app_fw()->init());

			auto framework_guard = sg::make_scope_guard([]() {
				if (auto res = imgui_app_fw()->destroy(); !res)
				{
					// report error
				}
			});

			BOOST_LEAF_CHECK(imgui_app_fw()->set_window_title("Hello!"));
			...
			while (!done)
			{
				BOOST_LEAF_AUTO(pump_result, imgui_app_fw()->pump());
				if (pump_result)
				{
					BOOST_LEAF_CHECK(imgui_app_fw()->begin_frame());
					auto frame_guard = sg::make_scope_guard([&clear_color]() {
						if (auto res = imgui_app_fw()->end_frame(clear_color); !res)
						{
							// report error
						}
					});
